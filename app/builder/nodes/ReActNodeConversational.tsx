...
const inputHandles = [
  ...
  {
    label: "Memory",
    acceptedType: NodeDataTypes.Memory,
  },
];
...
export default memo(({ id, data }: NodeComponentProps) => {
  ...
  const incomingMemoryConns = useNodeConnections({
    id: id,
    handleType: "target",
    handleId: NodeDataTypes.Memory,
  });

  const llmNodeData = useNodesData(incomingLLMConns[0]?.source);
  const memoryNodeData = useNodesData(incomingMemoryConns[0]?.source);
  ...
  const run = async () => {
    ...
    const promptTemplate = await pull<PromptTemplate>("hwchase17/react-chat");
